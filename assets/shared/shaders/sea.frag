...
vec3 color(vec3 p,vec3 norm, vec3 d)
{
    ...
    float S = pow(clamp(dot(reflect(norm,d),normalize(light-p)),0.0,1.0)*0.5*0.5,8.0);
    float f = 1.0-pow(srand(vec3(floor(p.x/lights))),4.0)*srand(vec3(iTime*8.0))-cos(iTime*8.0)*iTime*0.5;
...
void main()
{
    ...
    vec3 p = vec3(iTime*5.,Y-2.,X);//3D Position
    ...
    if (r>0.0)
    {
    	c = mix(color(p+d*r,calcNormal(p+d*r),d),c,pow(r*40.0,2.0));//Material colozr and fade
    }
