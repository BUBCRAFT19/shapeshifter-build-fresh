...
class PlayState extends MusicBeatState
{
	...
	var noDodgeCounter:Int = 0;
	var blackShit:FlxSprite;
	var defaultZoom:Float;
	var redLight:FlxRuntimeShader;
	var blurSize:Float = 0;
	var defaultAngle:Float = 0;
	var tweens:Array<FlxTween> = [];
	var spaceSprite:FlxSprite;
	var inTanga:Bool = false;
	var messages:Array<String> = [
		"BE MORE CAREFUL NEXT TIME!",
		"THIS IS YOUR LAST CHANCE, TAKE ADVANTAGE OF IT!!",
		"Oops, Better luck next time..."
	];
	var goodMessages:Array<String> = ["Good!", "Nice Job!", "You're doing great!"];
	var sexo:FlxText;
	var shit:FlxSprite;
	...
	// sida
	var sea:FlxRuntimeShader;
	var seaSprite:FlxSprite;
	var otherOppo:Character;
	var icon:HealthIcon;
	...
	override public function create()
	{
		...
		blackShit = new FlxSprite().makeGraphic(Std.int(FlxG.width * 1.5), Std.int(FlxG.height * 1.5), FlxColor.BLACK);
		blackShit.screenCenter();
		blackShit.scrollFactor.set();
		blackShit.alpha = 0;
		addBehindBF(blackShit);

		defaultZoom = defaultCamZoom;

		if (ClientPrefs.data.shaders)
		{
			redLight = new FlxRuntimeShader(File.getContent('assets/shared/shaders/redLight.frag'));
			blur = new FlxRuntimeShader(File.getContent('assets/shared/shaders/blur.frag'));
			redLight.setFloat('red', 1);
			camGame.setFilters([new ShaderFilter(redLight), new ShaderFilter(blur)]);
		}

		spaceSprite = new FlxSprite().loadGraphic(Paths.image('space'));
		spaceSprite.scale.set(0.6, 0.6);
		spaceSprite.updateHitbox();
		spaceSprite.screenCenter(X);
		spaceSprite.y = FlxG.height - (spaceSprite.height / 2) - 200;
		spaceSprite.cameras = [camOther];
		...
		shit.loadGraphic(Paths.image('check'));
		shit.alpha = 0.00001;

		sexo = new FlxText(0, 0, FlxG.width * 0.9, "", 32);
		sexo.setFormat(Paths.font("vcr.ttf"), 32, 0x0, "center", FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		sexo.borderSize = 2;
		sexo.cameras = [camOther];
		sexo.screenCenter(X);
		sexo.y = FlxG.height - sexo.height - 100;
		sexo.alpha = 0;
		add(sexo);
		...
		for (j in eventNotes)
		{
			if (j.event == 'Dodge Mechanic')
				times.push(j.strumTime);
		}

		for (v in [unspawnNotes])
		{
			for (j in v)
			{
				for (time in times)
				{
					if (!j.gfNote
						&& !j.noAnimation
						&& j.strumTime > time - 100
						&& j.strumTime < time + (formatAnimTime(dad, 'pre-attack') + formatAnimTime(dad, 'attack')) + 100)
					{
						j.ignoreNote = true;
					}
				}
		...
		if (songName == "brine")
		{
			if (ClientPrefs.data.shaders)
			{
				shader = new FlxRuntimeShader(File.getContent('assets/shared/shaders/grayscaleBars.frag'));
				blur = new FlxRuntimeShader(File.getContent('assets/shared/shaders/blur.frag'));
				camGame.setFilters([new ShaderFilter(blur), new ShaderFilter(shader)]);
				shader.setBool('rojo', true);

				luz = new FlxSprite(0, 200).loadGraphic(Paths.image('luz'));
				luz.scrollFactor.set(0.9, 0.95);
				add(luz);
			}
		}

		if (songName == "double-kill")
		{
			if (ClientPrefs.data.shaders)
			{
				shader = new FlxRuntimeShader(File.getContent('assets/shared/shaders/scaleAberration.frag'));
				shader.setFloat('scale', 0.99);

				var shaders = [];
				if (camGame.filters != null && camGame.filters != [])
					shaders = camGame.filters;

				shaders.push(new ShaderFilter(shader));
				camGame.setFilters(shaders);
			}

			otherOppo = new Character(800, 620, 'purple', false);
			addBehindDad(otherOppo);

			icon = new HealthIcon(otherOppo.healthIcon);
			icon.cameras = [camHUD];
			icon.y = iconP2.y - 15;
		...
		if (songName == "torture")
		{
			lime = new Character(0, 450, "lime");
			lime.scrollFactor.x = 0.88;
			addBehindDad(lime);

			icon = new HealthIcon(lime.healthIcon);
			icon.cameras = [camHUD];
			icon.y = iconP2.y - 30;
			uiGroup.insert(uiGroup.members.indexOf(iconP2) - 1, icon);

			for (i in unspawnNotes)
			{
				if (i.noteType == 'Opponent 2 Sing')
					i.noAnimation = true;
			}
		}
	...
	public function startCountdown()
	{
		...
		if (ret != FunkinLua.Function_Stop)
		{
			...
			startTimer = new FlxTimer().start(Conductor.crochet / 1000 / playbackRate, function(tmr:FlxTimer)
			{
				...
				if (songName == "double-kill")
				{
					if (swagCounter % otherOppo.danceEveryNumBeats == 0
						&& !StringTools.startsWith(otherOppo.getAnimationName(), 'sing')
						&& !otherOppo.stunned)
						otherOppo.dance();
				}
				...
				{
					if (lime != null
						&& swagCounter % lime.danceEveryNumBeats == 0
						&& !StringTools.startsWith(lime.getAnimationName(), 'sing')
						&& !lime.stunned)
						lime.dance();
				}
	...
	function createDadGhost()
	{
		var ghost:FlxSprite = new FlxSprite(dad.x, dad.y).loadGraphicFromSprite(dad);
		ghost.animation.stop();
		ghost.scale.set(dad.scale.x, dad.scale.y);
		ghost.updateHitbox();
		ghost.offset.set(dad.offset.x, dad.offset.y);
		ghost.blend = BlendMode.ADD;
		// ghost.color = 0xFFF00000;
		ghost.acceleration.x = -700;
		ghost.velocity.x = 800;
		// ghost.scale.x = ghost.scale.y = (Math.sin(((ghost.x-dad.x)*0.01)*Math.PI)*0.5)+0.5;
		ghost.alpha = 0.5;
		FlxTween.tween(ghost, {alpha: 0}, 1, {ease: FlxEase.sineInOut});
	...
	override public function update(elapsed:Float)
	{
		...
		if (canDodge && !isDodging && FlxG.keys.justPressed.SPACE)
		{
			isDodging = true;
		...
		if (ClientPrefs.data.shaders)
		{
			blurSize = FlxMath.lerp(0, blurSize, FlxMath.bound(1 - (elapsed * 3.125 * playbackRate)));
			blur.setFloat('num', blurSize);
		}

		camGame.angle = FlxMath.lerp(defaultAngle, camGame.angle, FlxMath.bound(1 - (elapsed * 3.125 * playbackRate)));
		vocals.pan = Math.sin(iTime);
		iTime += elapsed;
		...
		if (songName == "brine")
		{
			...
			luz.alpha = (Math.sin(iTime * 2.5) * 0.5) + 0.5;

			if (ClientPrefs.data.shaders)
				blur.setFloat('num', (Math.sin(iTime * 2.5) * 0.75) + 0.25);
		}

		if (songName == "double-kill")
		{
			icon.scale.set(iconP2.scale.x - 0.2, iconP2.scale.x - 0.2);
			icon.x = iconP2.x - 40;
		}
		...
		{
			icon.x = iconP2.x - 40;
			icon.scale.x = icon.scale.y = iconP2.scale.x / 1.1;
		}
	}
		...
		if (((skipHealthCheck && instakillOnMiss) || health <= 0) && !practiceMode && !isDead)
		{
			...
			if (ret != FunkinLua.Function_Stop)
			{
				if (inTanga)
				{
					...
					sexo.text = messages[2];
					sexo.color = FlxColor.RED;
					sexo.alpha = 1;
					sexo.cameras = [camera];
					GameOverSubstate.instance.add(sexo);
					FlxTween.tween(sexo, {alpha: 0, y: sexo.y - 40}, 1.8, {
	...
	public function triggerEvent(eventName:String, value1:String, value2:String, strumTime:Float)
	{
		...
		switch (eventName)
		{
			...
			case "Dodge Mechanic":
				...
				tweens.push(FlxTween.tween(blackShit, {alpha: 0.5}, (formatAnimTime(dad, 'pre-attack') / 1000), {ease: FlxEase.sineInOut}));
				tweens.push(FlxTween.tween(this, {defaultCamZoom: 1.2}, (formatAnimTime(dad, 'pre-attack') / 1000) - 0.15, {ease: FlxEase.sineInOut}));
				tweens.push(FlxTween.tween(camFollow, {x: camFollow.x - 20}, (formatAnimTime(dad, 'pre-attack') / 1000) - 0.15, {ease: FlxEase.sineInOut}));

				if (ClientPrefs.data.shaders)
				{
					tweens.push(FlxTween.num(shader.getFloat('scale'), shader.getFloat('scale') - 0.02, (formatAnimTime(dad, 'pre-attack') / 1000), {
						ease: FlxEase.sineInOut
					}, (num:Float) ->
						{
							shader.setFloat('scale', num);
						}));
				}

				tweens.push(FlxTween.tween(camHUD, {alpha: 0.5}, 0.85, {ease: FlxEase.sineInOut}));
				...
				FlxG.sound.play(Paths.sound('sacar'), 0.75);

				spaceSprite.alpha = 1;
				new FlxTimer().start((formatAnimTime(dad, 'pre-attack') / 1000) / (16 / 2), (tmr:FlxTimer) ->
				{
					spaceSprite.visible = !spaceSprite.visible;

					if (tmr.loopsLeft == 0)
					{
						new FlxTimer().start((formatAnimTime(dad, 'pre-attack') / 1000) / 16, (tmr:FlxTimer) ->
						{
							spaceSprite.visible = !spaceSprite.visible;
							if (tmr.loopsLeft == 0)
							{
								shit.loadGraphic(Paths.image(isDodging ? 'check' : 'x'));
								shit.alpha = 1;
								spaceSprite.visible = true;
								shit.scale.set(1.2 - 0.35, 0.8 - 0.35);
								shit.screenCenter(X);
								shit.y = spaceSprite.y + (spaceSprite.height - shit.height) / 2;
								FlxTween.tween(shit, {alpha: 0}, 1, {ease: FlxEase.quintIn});
								FlxTween.tween(shit.scale, {x: 1 - 0.35, y: 1 - 0.35}, 0.5, {ease: FlxEase.sineOut});

								// FlxTween.tween(shit.scale, {x: 1.3-0.35, y: 0.7-0.35}, 1, {ease: FlxEase.quintIn, startDelay: 0.25});

								FlxTween.tween(spaceSprite, {alpha: 0}, 1, {ease: FlxEase.quintIn});
								FlxTween.tween(spaceSprite.scale, {x: spaceSprite.scale.x + 0.5, y: 0.25}, 1, {
				...
				new FlxTimer().start((formatAnimTime(dad, 'pre-attack') / 1000), (tmr:FlxTimer) ->
				{
					canDodge = false;

					tweens.push(FlxTween.tween(blackShit, {alpha: 0}, 0.85, {ease: FlxEase.sineOut}));

					moveCamera(false);
					tweens.push(FlxTween.tween(camFollow, {x: (dad.getMidpoint().x + 150) + (dad.cameraPosition[0] + opponentCameraOffset[0])}, 1.75, {
						ease: FlxEase.sineInOut,
						onComplete: (twn:FlxTween) ->
						{
							isCameraOnForcedPos = false;
					...
					tweens.push(FlxTween.num(2, 0, 1.75, {ease: FlxEase.backInOut}, (num:Float) ->
					{
						defaultAngle = num;
					}));
					// FlxTween.tween(camGame, {angle: 0}, 1.75, {ease: FlxEase.sineInOut});

					defaultCamZoom = defaultZoom;
					dad.playAnim('attack', true);
					dad.specialAnim = true;
					FlxG.sound.play(Paths.sound('matar'), 0.75);
					createDadGhost();
					...
					if (!isDodging)
					{
						...
						boyfriend.specialAnim = true;
						dodgeFails++;
						health -= dodgeFails <= 2 ? (0.5 * dodgeFails) : 2;
						updateScore();
						blurSize = 7;

						sexo.text = messages[dodgeFails - 1];
						sexo.color = FlxColor.RED;
						sexo.alpha = 1;
						FlxTween.tween(sexo, {alpha: 0, y: sexo.y - 40}, 1.8, {
							ease: FlxEase.quadOut,
					...
					{
						sexo.text = goodMessages[FlxG.random.int(1, goodMessages.length - 1)];
						sexo.color = FlxColor.LIME;
						sexo.alpha = 1;
						FlxTween.tween(sexo, {alpha: 0, y: sexo.y - 40}, 1.8, {
							ease: FlxEase.quadOut,
							onComplete: (twn:FlxTween) ->
					...
					if (ClientPrefs.data.shaders)
					{
						tweens.push(FlxTween.num(shader.getFloat('scale'), shader.getFloat('scale') + 0.02, 0.85, {ease: FlxEase.sineOut}, (num:Float) ->
						{
							shader.setFloat('scale', num);
						}));

						tweens.push(FlxTween.num(5, 1, 0.85, {ease: FlxEase.sineOut}, (num:Float) ->
						{
							redLight.setFloat('red', num);
						}));
					}

					tweens.push(FlxTween.tween(camHUD, {alpha: 1}, 0.85, {ease: FlxEase.sineInOut}));
	...
	function formatAnimTime(obj, animname) // dad.animation.curAnim.frames.length
	{
		return (obj.animation.getByName(animname).frames.length / Std.int(obj.animation.getByName(animname).frameRate)) * 1000;
	}
	...
	function opponentNoteHit(note:Note):Void
	{
		...
		else if (!note.noAnimation)
		{
			...
			if (songName == "double-kill")
			{
				...
				else if (note.noteType == 'White and Black duet')
				{
					otherOppo.holdTimer = 0;
					otherOppo.playAnim(animToPlay, true);
					char = dad;
				}
			}
			else if (note.gfNote)
		...
		var result:Dynamic = callOnLuas('opponentNoteHitPost', [
			notes.members.indexOf(note),
			Math.abs(note.noteData),
			note.noteType,
			note.isSustainNote
	...
	override function beatHit()
	{
		...
		if (songName == "double-kill")
		{
			if (curBeat % otherOppo.danceEveryNumBeats == 0
				&& !StringTools.startsWith(otherOppo.getAnimationName(), 'sing')
				&& !otherOppo.stunned)
				otherOppo.dance();
		}
		...
		{
			if (lime != null
				&& curBeat % lime.danceEveryNumBeats == 0
				&& !StringTools.startsWith(lime.getAnimationName(), 'sing')
				&& !lime.stunned)
				lime.dance();
		}
